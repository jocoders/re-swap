...
contract ReSwapCore {
  ...
  function getBalances(address tokenA) internal view returns (uint256 _balance0, uint256 _balance1) {
    if (isToken0(tokenA)) {
      _balance0 = getBalance0();
      _balance1 = getBalance1();
    } else {
  ...
  function getReserves(address tokenA) internal view returns (uint112 reserveA, uint112 reserveB) {
  ...
  function checkReserves(uint256 amount0, uint256 r0, uint256 r1) internal pure {
    assembly {
  ...
  function getTokenBalance(uint256 slot) private view returns (uint256 bal) {
    assembly {
      ...
      if iszero(success) {
        revert(0, 0)
      } // Если вызов неуспешен, делаем реверт
