...
contract ReSwapPair is ReSwapCore, ERC20, ReentrancyGuard, IReSwapFlashLender {
  ...
  function removeLiquidity(
    ...
    address to,
    uint256 deadline
  ) public nonReentrant returns (uint256 burnAmount0, uint256 burnAmount1) {
  ...
  function _transferPair(address tokenA, address tokenB, uint256 amountOutA, uint256 amountOutB, address to) private {
  ...
  function _removeLiquidity(
    ...
    address to,
    uint256 deadline
  ) private returns (uint256 burnAmount0, uint256 burnAmount1) {
    validateDeadline(deadline);
